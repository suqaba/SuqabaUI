...
def _getBesideDir(solver):
    ...
    specificPath = os.path.join(base)
...
def _getCustomDir(solver):
    ...
    specificPath = os.path.join(base, solver.Document.Name)
...
class Machine(BaseTask):
    ...
    def reset(self, newState=PREPARE):
...
class _DocObserver:
    ...
    def _deleteMachine(self, obj):
        m = _machines[obj]
        t = None
        if m.directory in _dirTypes:
            t = _dirTypes[m.directory]
            del _dirTypes[m.directory]
        m.abort()
        if t and t == settings.DirSetting.TEMPORARY:
